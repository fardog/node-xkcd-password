...
xkcd.Password.prototype._generate = function(numWords, minLength, maxLength, next, deferred) {
  ...
  debug('_generate: numWords, minLength, maxLength: ', numWords, minLength, maxLength);
  // ensure that required parameters have been set
  if (numWords <= 0 || minLength < 0 || maxLength < 2) {
    debug('_generate: parameters provided were not correct');
  ...
  // make sure we're not asking for more unique words than we have available
    ...
    debug('_generate: More words than were available in the wordlist were requested.');
    ...
    debug('_generate: Your maximum word length can\'t be less than your minimum length. Try specifying a maximum length and minimum length directly.');
  ...
  // generate the numbers
    ...
    debug('_generate: validation passed generating the numbers');
    ...
    rand.randomInts({unique: true, num: numWords, min: 0, max: self.wordlist.length}, function generateWords(err, ints) {
      if (err) {
        debug('_generate: error generating random Ints: ', err);
        next(err);
        if (deferred) {
          deferred.reject(err);
        }
      ...
      function resolveGeneration(err) {
        debug('_generate.resolveGeneration: err: ', err);
        if (err) {
          next(err);
          if (deferred) {
            deferred.reject(err);
          ...
          debug('_generate.resolveGeneration: You asked for more words than could be generated. This may because you\'re asking for too many words of a certain length.');
          ...
          debug('_generate.resolveGeneration: success - words: ', words);
          next(null, words);
          if (deferred) {
